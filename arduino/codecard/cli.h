...
void help() {
    ...
    Serial.println(
        F("  longpress[a|b]    Simulate the long press of a button"));
...
void evalInput(String input) {
    ...
    if (keyIndex == -1) {
        ...
        Serial.print(key);
        Serial.print(F("'"));
        Serial.println(F(" is not a valid key or command! Type 'help' or 'ls' "
                         "for more info."));
        Serial.println(F(">>>"));
        return;
    }
    ...
    if (val != "") {
        ...
        if (key == "ssid" || key == "password") {
            Serial.println(
                F("Use the command 'restart' for changes to take effect. "));
            Serial.println(F(">>>"));
            return;
        }
    } else {
